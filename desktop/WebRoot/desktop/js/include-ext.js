...
(function() {
    ...
    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + path + '/ext4.2/resources/css/ext-all' + suffix + '-debug.css"/>');
    }
    document.write('<script type="text/javascript" src="' + path + '/ext4.2/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    ...
    if (neptune) {
        ...
        neptunePath = (repoDevMode ? path + '/..' : path) +
            '/ext4.2/packages/ext-theme-neptune/build/ext-theme-neptune' +
            (repoDevMode ? '-dev' : '') + '.js';
